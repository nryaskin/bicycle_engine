...
namespace wayland::generator {
    ...
    cpp::Type Builder::wire_to_type(wire_type type) {
        switch (type) {
            case wire_type::FD:
                static cpp::Type wire_fd_type(ns_prefix + "wire_fd_t");
                return wire_fd_type;
                break;
            case wire_type::FIXED:
                static cpp::Type wire_fixed_type(ns_prefix + "wire_fixed_t");
                return wire_fixed_type;
                break;
            case wire_type::INT:
                static cpp::Type wire_int_type(ns_prefix +"wire_int_t");
                return wire_int_type;
                break;
            case wire_type::UINT:
                static cpp::Type wire_uint_type(ns_prefix + "wire_uint_t");
                return wire_uint_type;
                break;
            case wire_type::NEW_ID:
                static cpp::Type wire_new_id_type(ns_prefix + "wire_new_id_t");
                return wire_new_id_type;
                break;
            case wire_type::OBJECT:
                static cpp::Type wire_object_type(ns_prefix + "wire_object_id_t");
                return wire_object_type;
                break;
            case wire_type::ARRAY:
                static cpp::Type wire_array_type(ns_prefix + "wire_array_t");
                return wire_array_type;
                break;
            case wire_type::STRING:
                static cpp::Type wire_string_type(ns_prefix + "wire_string_t");
    ...
    std::vector<cpp::Parameter> Builder::gen_parameters(const std::vector<WLArgument>& args) {
        ...
        for (const auto& arg : args) {
            ...
            cpp::Parameter param(wire_to_type(arg.type), arg.name);
    ...
    std::vector<cpp::SimpleDeclaration> Builder::gen_variables(const WLInterface& interface) {
        ...
        static cpp::Type op_code_type(ns_prefix + "wire_op_t");
        int counter = 0;
